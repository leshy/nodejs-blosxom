...
(function() {
  ...
  initLogger = function(env, callback) {
    env.log = function() {
      ...
      return console.log(String(new Date()) + " " + _.keys(tags).join(', ') + " " + text + json);
    };
  ...
  Wiki = Backbone.Model.extend4000({
    initialize: function() {
      return this.when('dir', (function(_this) {
        return function(dir) {
          _this.watchDir(dir);
    ...
    getPost: function(search, callback) {
      var found;
      found = false;
      return env.blog.findModel(search, function(err, post) {
        if (post) {
          if (!found) {
            found = true;
    ...
    getPosts: function(search, callback, callbackDone) {
    ...
    getPostsByTags: function(search, tags_yes, tags_no, callback, callbackDone) {
      var query;
      if (search == null) {
        search = {};
      }
    ...
    parseFile: function(file) {
      var data, error, manualOptions, match, newline, options, pathtags, stat, tags, writeOptions;
      try {
        stat = fs.statSync(file);
  ...
  initRoutes = function(callback) {
    ...
    parseTagsString = function(tags) {
      ...
      tags = tags.replace('+', ' +');
      tags = tags.replace('-', ' -');
      tags_yes = helpers.makeDict(tags.match(/(?:\+)(\w*)\w/g), function(tag) {
        return tag.replace('+', '');
      });
      tags_no = helpers.makeDict(tags.match(/\-(\w*)\w/g), function(tag) {
        return tag.replace('-', '');
      });
    ...
    servetags = function(tags_yes, tags_no, key, outputType, res, req) {
      ...
      return env.wiki.getPostsByTags({}, tags_yes, tags_no, (function(err, post) {
        var posttags;
        if (post) {
